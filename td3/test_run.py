        ...
        for v in range(len(values)):
            ...
            for j in objective:
                if j == "None":
                    ...
                    run_command = "python train_td3.py --load {0}  --env {1} --gpu -1 --seed {2} --objective {3} --attack {4} --space {5} --direction {6} --percentage {7}".format(values[v],key+"-v2",v,j,'_','_','_',0)
                ...
                for u in attack:
                    if u == "range":
                        for ii in percentage:
                            ...
                            run_command = "python train_td3.py --load {0}  --env {1} --gpu -1 --seed {2} --objective {3} --attack {4} --space {5} --direction {6} --percentage {7}".format(values[v],key+"-v2",v,j,u,'_','_',ii)
                        ...
                        for l in direction:
                            ...
                            for d in percentage:
                                if k =="aspace" and j == 'obs' and key == "Ant" and l in ['same','flip']:
                                    for ze in zeros:
                                        run_command = "python train_td3.py --load {0}  --env {1} --gpu -1 --seed {2} --objective {3} --attack {4} --space {5} --direction {6} --percentage {7} --zeros {8}".format(values[v],key+"-v2",v,j,u,k,l,d,ze)
                                    
                                        test_run.append(run_command)
                                    continue

                                run_command = "python train_td3.py --load {0}  --env {1} --gpu -1 --seed {2} --objective {3} --attack {4} --space {5} --direction {6} --percentage {7}".format(values[v],key+"-v2",v,j,u,k,l,d)
