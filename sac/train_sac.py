...
from White_Attacker_SAC import FGSM, PGD
start = time.time()
...
def main():
    ...
    def make_env(test, process_idx=0 ):
    ...
    if args.load:
                ...
                while True:
                    ...
                    if args.rollout == 'MAS':
                        ...
                        if t < args.start_atk:
                            ...
                            #env.render()
                            action_dist = spy.act_forward(obs)
                            action_basedist = action_dist.base_dist
                            transform = action_dist.transforms
                            
                            action_mean = mode_of_distribution(action_dist)
                            action_var = transform[0](action_basedist.variance.data[0])
                            ...
                            means = []
                            std_devs = []
                            for k in range(len(action_mean.data[0])):
                                means.append(np.array(action_mean[0][k].data))
                                var = np.exp(np.array(action_var[k].data))
        ...
        env = make_env(test=False,process_idx=0)
        obs = env.reset()
        phi = lambda x: x.astype(np.float32, copy=False)
        obs = phi(obs)
