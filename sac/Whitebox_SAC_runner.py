            ...
            for j in rollout:
                for l in epsilon:
                    if j == "FGSM":
                        ...
                        run_command = "python train_sac.py --load {0}  --env {1} --gpu -1 --seed {2} --rollout {3} --epsilon {4}".format(values[v],key+"-v2",v,j, l)
                        ...
                        for d in lr:
                            if j == "PGD":
                                for stp in steps_number: 
                                    ...
                                    run_command = "python train_sac.py --load {0}  --env {1} --gpu -1 --seed {2} --rollout {3} --epsilon {4} --clip {5} --lr {6} --attack_steps {7}".format(values[v],key+"-v2",v,j, l,u, d, stp)
                            ...
                            for k in start_atk:
                                    for ze in budget:
                                        for s in norm:
                                            run_command = "python train_sac.py --load {0}  --env {1} --gpu -1 --seed {2} --rollout {3} --clip {4} --start_atk {5} --epsilon {6} --lr {7} --budget {8} --s {9}".format(values[v],key+"-v2",v,j,u,k,l,d,ze,s)
